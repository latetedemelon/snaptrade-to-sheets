...
### Accounts Sheet
Generated by **📊 SnapTrade → 📊 Refresh Accounts**

| Column | Description | Visibility |
|--------|-------------|------------|
| Account Name | The name or number of the account | Visible |
| Balance | Account balance for this currency (currency formatted) | Visible |
| Currency | Currency code (e.g., USD, CAD) | Visible |
| Notes | Placeholder for manual notes | Visible |
| Last Update | Last successful sync timestamp | Visible |
| Institution | Brokerage institution name | Visible |
| Account ID | Unique account identifier | Hidden by default |
| Raw Data | Complete JSON response for reference | Hidden by default |

**Important:** Each account with multiple currency balances (e.g., IKBR with USD and CAD) will appear as separate rows. This ensures all balances are visible.

### Transactions Sheet
Generated by **📊 SnapTrade → 📜 Refresh Transactions**

| Column | Description | Visibility |
|--------|-------------|------------|
| Date | Transaction or settlement date | Visible |
| Amount | Transaction amount (currency formatted) | Visible |
| Description | Transaction description | Visible |
| Category | Transaction type (BUY, SELL, DIVIDEND, etc.) | Visible |
| Account | Account name or number | Visible |
| Attachment | Placeholder for manual attachments | Visible |
| Transaction ID | Unique transaction identifier | Hidden by default |
| Raw Data | Complete JSON response for reference | Hidden by default |

### Holdings Sheet
Generated by **📊 SnapTrade → 💰 Refresh Holdings**
...
Auto-updated by **📊 SnapTrade → 📊 Refresh Accounts** (once per day)

| Column | Description |
|--------|-------------|
| Timestamp | Date when the snapshot was taken (yyyy-mm-dd format) |
...
This sheet builds a time-series record of account values. When you refresh accounts, it automatically updates (once per day). Multiple refreshes in the same day update existing rows instead of creating duplicates.

**Tip:** The timestamp format has been simplified to yyyy-mm-dd for cleaner visualization and charting.

**Note:** Hidden columns (Account ID, Raw Data, Transaction ID) can be unhidden when needed for debugging or advanced usage. All data sheets now feature formatted headers with color coding, frozen header rows, and auto-sized columns for improved readability.
